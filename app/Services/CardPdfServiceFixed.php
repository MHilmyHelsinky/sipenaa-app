...
class CardPdfServiceFixed
{
    ...
    // Exact stamp/signature geometry from the original Word artwork.
    ...
    // The template already contains "Banda Aceh," and the official block.
    // Only the current print date is appended after "Banda Aceh,".
    ...
    public function render(Card $card): string
    {
        ...
        if (! is_file($stamp) || ! is_file($signature)) {
            throw new RuntimeException(
                'stamp.png dan signature.png harus tersedia di storage/app/templates/.'
            );
        }
        ...
        // The template already contains "Banda Aceh,". Add only today's print date.
        ...
        // Correct layer order: photo -> stamp -> signature.
