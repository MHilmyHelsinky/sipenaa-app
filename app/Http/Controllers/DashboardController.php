...
class DashboardController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($user->role === 'user') {
            ...
            $inputTodayData = Card::query()->whereBetween('created_at', [$todayStartUtc, $todayEndUtc])->latest('created_at')->get()->map(fn (Card $card) => [
                'nisn' => $card->nisn,
    ...
    public function storeCard(Request $request)
    {
        ...
        $batchIds = collect($request->session()->get('card_preview_ids', []))->map(fn ($id) => (int) $id)->filter()->values();
        if (! $batchIds->contains($card->id)) {
            $batchIds->push($card->id);
        }
        $request->session()->put('card_preview_ids', $batchIds->values()->all());
    ...
    public function previewKartu(Request $request, Card $card): View
    {
        $batchIds = collect($request->session()->get('card_preview_ids', []))->map(fn ($id) => (int) $id)->filter()->unique()->values();
        if (! $batchIds->contains($card->id)) {
            $batchIds->push($card->id);
            $request->session()->put('card_preview_ids', $batchIds->all());
        }
        $previewCards = Card::whereIn('id', $batchIds)->orderBy('id')->get();
        return view('preview-kartu', ['card' => $card, 'previewCards' => $previewCards, 'previewCount' => $previewCards->count()]);
    }

    public function previewPdf(Card $card)
    {
        $path = $this->renderPdf($card);
        return response()->file($path, ['Content-Type' => 'application/pdf', 'Content-Disposition' => 'inline; filename="kartu_' . Str::slug($card->nama_lengkap ?: 'siswa') . '_' . ($card->nisn ?: 'card') . '.pdf"']);
    }

    public function previewBatchPdf(Request $request)
    {
        $ids = collect($request->session()->get('card_preview_ids', []))->map(fn ($id) => (int) $id)->filter()->unique()->values();
        if ($ids->isEmpty()) {
            abort(404, 'Belum ada kartu di daftar preview.');
        }
        $cards = Card::whereIn('id', $ids)->orderBy('id')->get();
        $path = app(BatchCardPdfService::class)->render($cards, 5);
        return response()->file($path, [
            'Content-Type' => 'application/pdf',
            'Content-Disposition' => 'inline; filename="preview-kartu-satu-lembar.pdf"',
        ])->deleteFileAfterSend(true);
    }

    public function clearPreviewBatch(Request $request)
    {
        $request->session()->forget('card_preview_ids');
        return redirect()->route('pembuatan.kartu');
    }
    ...
    {
        $validated = $request->validate(['card_ids' => ['required', 'array', 'min:1'], 'card_ids.*' => ['integer', 'exists:cards,id'], 'per_page' => ['nullable', 'integer', 'in:4,5']]);
        $cards = Card::whereIn('id', $validated['card_ids'])->orderBy('id')->get();
        $perPage = (int) ($validated['per_page'] ?? 5);
        $path = app(BatchCardPdfService::class)->render($cards, $perPage);
    ...
    public function dataKartu(Request $request): View
    {
        ...
        $query = Card::query();
        if ($search !== '') $query->where(fn ($q) => $q->where('nisn', 'like', '%' . $search . '%')->orWhere('nama_lengkap', 'like', '%' . $search . '%'));
        if ($waktuInput !== '') $query->whereDate('created_at', Carbon::parse($waktuInput)->format('Y-m-d'));
        return view('data-kartu', ['cards' => $query->latest()->get(), 'search' => $search, 'waktuInput' => $waktuInput]);
    }

    public function laporan(): View
    {
        $cards = Card::query()->latest('created_at')->get()->map(fn (Card $card) => [
            'id' => $card->id, 'nisn' => $card->nisn ?? '-', 'nama_lengkap' => $card->nama_lengkap ?? '-', 'tempat_lahir' => $card->tempat_lahir ?? '-',
            'tanggal_lahir' => optional($card->tanggal_lahir)->format('d-m-Y') ?? '-', 'alamat' => trim(($card->desa ? $card->desa . ', ' : '') . ($card->kecamatan ? $card->kecamatan . ', ' : '') . ($card->kabupaten ?? '')) ?: '-',
    ...
    public function exportLaporan()
    {
        ...
        $spreadsheet = new Spreadsheet(); $sheet = $spreadsheet->getActiveSheet(); $sheet->setTitle('Laporan');
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'laporan_'); (new Xlsx($spreadsheet))->save($tempFile);
    ...
    public function storeUser(Request $request) { $this->authorizeSuperAdmin(); $v = $request->validate(['name' => ['required','string','max:255'],'password' => ['required','confirmed','min:8'],'role' => ['required','in:super_admin,user']]); User::create(['name'=>$v['name'],'username'=>$this->generateUniqueUsername($v['name']),'password'=>Hash::make($v['password']),'role'=>$v['role'],'is_active'=>true]); return back()->with('success','Akun baru berhasil dibuat.'); }
